...
if NBB:
    if True:
        ...
        for i in range(A_corres.shape[0]):
            plt.plot(A_corres[i][1], A_corres[i][0], marker='o', markersize=5)
    ...
    if True:
        ...
        for i in range(B_corres.shape[0]):
            plt.plot(B_corres[i][1], B_corres[i][0], marker='o', markersize=5)
        plt.gca().set_axis_off()
        plt.subplots_adjust(top=1, bottom=0, right=1, left=0, hspace=0, wspace=0)
        plt.margins(0, 0)
        plt.gca().xaxis.set_major_locator(plt.NullLocator())
...
if True:
    ...
    plt.imshow(A_im)
    for i in range(A_selected.shape[0]):
        plt.plot(A_selected[i][1], A_selected[i][0], marker='o', markersize=5, color=colors[i])
        plt.plot(B_selected_new[i][1], B_selected_new[i][0], marker='s', markersize=5, color=colors[i])
        plt.arrow(A_selected[i][1], A_selected[i][0], B_selected_new[i][1]-A_selected[i][1], B_selected_new[i][0]-A_selected[i][0], color=colors[i], width=0.05)
    plt.gca().set_axis_off()
    plt.subplots_adjust(top=1, bottom=0, right=1, left=0, hspace=0, wspace=0)
    plt.margins(0, 0)
    plt.gca().xaxis.set_major_locator(plt.NullLocator())
...
if True:
    ...
    for i in range(B_selected.shape[0]):
        plt.plot(B_selected[i][1], B_selected[i][0], marker='o', markersize=5, color=colors[i])
    plt.gca().set_axis_off()
    plt.subplots_adjust(top=1, bottom=0, right=1, left=0, hspace=0, wspace=0)
    plt.margins(0, 0)
    plt.gca().xaxis.set_major_locator(plt.NullLocator())
